...
from util.util_PST_900 import compute_results
from sklearn.metrics import confusion_matrix
from torch.utils.tensorboard import SummaryWriter
from pytorch_toolbelt import losses as L
from loss_hub.losses import DiceLoss,SoftCrossEntropyLoss
from model.CrissCrossAttention_dual_2_sinINF_PST import FEANet
...
parser.add_argument('--data_dir', '-dr', type=str, default='/workspace/PST900_RGBT_Dataset/')
...
def train(epo, model, train_loader, optimizer):
    ...
    for it, (images, thermal, labels, names) in enumerate(train_loader):
        ...
        criterion = L.JointLoss(first=DiceLoss_fn, second=SoftCrossEntropy_fn,first_weight=0.4, second_weight=0.6).cuda()
        ...
        if accIter['train'] % 500 == 0:
            if view_figure:
                input_rgb_images = vutils.make_grid(images[:, :3], nrow=4,
                ...
                groudtruth_images = vutils.make_grid(groundtruth_tensor, nrow=4, padding=10)
                ...
                predicted_images = vutils.make_grid(predicted_tensor, nrow=4, padding=10)
...
def testing(epo, model, test_loader):
    ...
    label_list = ["background", "hand-drill", "backpack", "fire-extinguisher", "survivor"]
    ...
    if epo == 0:
        with open(testing_results_file, 'w') as f:
            ...
            f.write(
                "# epoch: unlabeled, hand-drill, backpack, fire-extinguisher, survivor, average(nan_to_num). (Acc %, IoU %)\n")
...
if __name__ == '__main__':
    ...
    weight_dir = os.path.join("./runs_dual_2.2_PST900_batch2_lr0.01_loss0.4_0.6/", args.model_name)
    if not os.path.exists(weight_dir):
        os.makedirs(weight_dir)
    os.chmod(weight_dir,
             stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine

    writer = SummaryWriter("./runs_dual_2.2_PST900_batch2_lr0.01_loss0.4_0.6/tensorboard_log")
    os.chmod("./runs_dual_2.2_PST900_batch2_lr0.01_loss0.4_0.6/tensorboard_log",
             stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine
    os.chmod("./runs_dual_2.2_PST900_batch2_lr0.01_loss0.4_0.6", stat.S_IRWXO)

    print('training %s on GPU #%d with pytorch' % (args.model_name, args.gpu))
    print('from epoch %d / %s' % (args.epoch_from, args.epoch_max))
    print('weight will be saved in: %s' % weight_dir)

    train_dataset = PST_dataset(data_dir=args.data_dir+"train/", split='train', transform=augmentation_methods)
    val_dataset = PST_dataset(data_dir=args.data_dir+"test/", split='val')
    test_dataset = PST_dataset(data_dir=args.data_dir+"test/", split='test_a')
