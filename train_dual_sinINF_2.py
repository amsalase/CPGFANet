...
def train(epo, model, train_loader, optimizer):
    ...
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        criterion = L.JointLoss(first=DiceLoss_fn, second=SoftCrossEntropy_fn,first_weight=0.6, second_weight=0.5).cuda()
...
if __name__ == '__main__':
    ...
    weight_dir = os.path.join("./runs_dual_2.2.dice0.6_ce0.5/", args.model_name)
    if not os.path.exists(weight_dir):
        os.makedirs(weight_dir)
    os.chmod(weight_dir,
             stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine

    writer = SummaryWriter("./runs_dual_2.2.dice0.6_ce0.5/tensorboard_log")
    os.chmod("./runs_dual_2.2.dice0.6_ce0.5/tensorboard_log",
             stat.S_IRWXO)  # allow the folder created by docker read, written, and execuated by local machine
    os.chmod("./runs_dual_2.2.dice0.6_ce0.5", stat.S_IRWXO)
