...
class CrissCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        new_x = self.gamma*(out_H + out_W) + x 
        return new_x
...
class RCCAModule(nn.Module):
    def __init__(self, in_channels):
        ...
        self.conva = nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False)
        self.cca = CrissCrossAttention(inter_channels)
        self.sa = SpatialAttention()
        self.convb = nn.Conv2d(1, inter_channels, 3, padding=1, bias=False)
...
class FEANet(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        self.atten_RCCAModule_0 = RCCAModule(128)
        ...
        #######  Fusion ##########
        self.fusion_maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.fusion_layer1 = nn.Conv2d(64, 256, kernel_size=3, padding=1, bias=False)
        self.fusion_layer2 = nn.Conv2d(256, 512, kernel_size=3, padding=1, bias=False)
        self.fusion_layer3 = nn.Conv2d(512, 1024, kernel_size=3, padding=1, bias=False)
        self.fusion_layer4 = nn.Conv2d(1024, 2048, kernel_size=3, padding=1, bias=False)
    ...
    def forward(self, input):
        ...
        #print(rgb.size())
        #print(thermal.size())
        #fuse_1 = torch.cat((rgb,thermal),1)
        #print(fuse_1.size())
        #fuse_1a  = self.atten_RCCAModule_0(fuse_1)
        #rgb = fuse_1 + fuse_1a
        #print(rgb.size())
        ######################################################################
        fuse1 = rgb + thermal #comentar para usar los bloques RCCAModule en imagen termica y RGB
        fuse1 = self.fusion_maxpool(fuse1)
        fuse1 = self.fusion_layer1(fuse1)
        print(fuse1.size())
        ######################################################################
        ...
        ######################################################################
        fuse2 = fuse1 + rgb + thermal
        fuse2 = self.fusion_maxpool(fuse2)
        fuse2 = self.fusion_layer2(fuse2)
        #fuse_2 = torch.cat((rgb,thermal),1)
        #fuse_2a  = self.atten_RCCAModule_1(fuse_2)
        #rgb = fuse_2 + fuse_2a
        ######################################################################
        ...
        ######################################################################
        #fuse_3 = torch.cat((rgb,thermal),1)
        #fuse_3a  = self.atten_RCCAModule_2(fuse_3)
        #rgb = fuse_3 + fuse_3a
        fuse3 = fuse2 + rgb + thermal
        fuse3 = self.fusion_maxpool(fuse3)
        fuse3 = self.fusion_layer3(fuse3)
        ######################################################################
        ...
        ######################################################################
        #fuse_4 = torch.cat((rgb,thermal),1)
        #fuse_4a  = self.atten_RCCAModule_3(fuse_4)
        #rgb = fuse_4 + fuse_4a
        fuse4 = fuse3 + rgb + thermal
        fuse4 = self.fusion_maxpool(fuse4)
        fuse4 = self.fusion_layer4(fuse4)
        ######################################################################
        ...
        ######################################################################
        # Des-comentar para usar los bloques RCCAModule en imagen termica y RGB
        #fuse_5 = torch.cat((rgb,thermal),1)
        #fuse_5a  = self.atten_RCCAModule_4(fuse_5)
        #fuse = fuse_5 + fuse_5a
        ######################################################################
        fuse = fuse4 + rgb + thermal #comentar para usar los bloques RCCAModule en imagen termica y RGB
        ######################################################################
        # decoder
