...
def INF(B,H,W):
    return -torch.diag(torch.tensor(float("inf"), device=2).repeat(H),0).unsqueeze(0).repeat(B*W,1,1)
...
class FEANet(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        self.ConvDe53 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1)
        self.BNDe53 = nn.BatchNorm2d(2048, momentum=BN_momentum)
        self.ConvDe52 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1)
        self.BNDe52 = nn.BatchNorm2d(2048, momentum=BN_momentum)
        self.ConvDe51 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1)
        self.BNDe51 = nn.BatchNorm2d(2048, momentum=BN_momentum)

        self.ConvDe43 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1)
        self.BNDe43 = nn.BatchNorm2d(2048, momentum=BN_momentum)
        self.ConvDe42 = nn.Conv2d(2048, 2048, kernel_size=3, padding=1)
        self.BNDe42 = nn.BatchNorm2d(2048, momentum=BN_momentum)
        self.ConvDe41 = nn.Conv2d(2048, 1024, kernel_size=3, padding=1)
        self.BNDe41 = nn.BatchNorm2d(1024, momentum=BN_momentum)

        self.ConvDe33 = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.BNDe33 = nn.BatchNorm2d(1024, momentum=BN_momentum)
        self.ConvDe32 = nn.Conv2d(1024, 1024, kernel_size=3, padding=1)
        self.BNDe32 = nn.BatchNorm2d(1024, momentum=BN_momentum)
        self.ConvDe31 = nn.Conv2d(1024, 512, kernel_size=3, padding=1)
        self.BNDe31 = nn.BatchNorm2d(512, momentum=BN_momentum)

        self.ConvDe23 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.BNDe23 = nn.BatchNorm2d(512, momentum=BN_momentum)
        self.ConvDe22 = nn.Conv2d(512, 512, kernel_size=3, padding=1)
        self.BNDe22 = nn.BatchNorm2d(512, momentum=BN_momentum)
        self.ConvDe21 = nn.Conv2d(512, 64, kernel_size=3, padding=1)
        self.BNDe21 = nn.BatchNorm2d(64, momentum=BN_momentum)
              
        self.ConvDe02 = nn.Conv2d(64, 64, kernel_size=3, padding=1)
        self.BNDe02 = nn.BatchNorm2d(64, momentum=BN_momentum)
        self.ConvDe01 = nn.Conv2d(64, 9, kernel_size=3, padding=1)
        self.BNDe01 = nn.BatchNorm2d(9, momentum=BN_momentum)
    ...
    def forward(self, input):
        ...
        # decoder
        #Stage 5
        fuse = F.interpolate(fuse, size=stage5.size()[2:],mode='bilinear', align_corners=True)
        fuse = F.relu(self.BNDe53(self.ConvDe53(fuse)))
        fuse = F.relu(self.BNDe52(self.ConvDe52(fuse)))
        fuse = F.relu(self.BNDe51(self.ConvDe51(fuse)))

        #Stage 4
        fuse = F.interpolate(fuse, size=stage4.size()[2:],mode='bilinear', align_corners=True)
        fuse = F.relu(self.BNDe43(self.ConvDe43(fuse)))
        fuse = F.relu(self.BNDe42(self.ConvDe42(fuse)))
        fuse = F.relu(self.BNDe41(self.ConvDe41(fuse)))

        #Stage 3
        fuse = F.interpolate(fuse, size=stage3.size()[2:],mode='bilinear', align_corners=True)
        fuse = F.relu(self.BNDe33(self.ConvDe33(fuse)))
        fuse = F.relu(self.BNDe32(self.ConvDe32(fuse)))
        fuse = F.relu(self.BNDe31(self.ConvDe31(fuse)))

        #Stage 2
        fuse = F.interpolate(fuse, size=stage2.size()[2:],mode='bilinear', align_corners=True)
        fuse = F.relu(self.BNDe23(self.ConvDe23(fuse)))
        fuse = F.relu(self.BNDe22(self.ConvDe22(fuse)))
        fuse = F.relu(self.BNDe21(self.ConvDe21(fuse)))

        #Stage 0
        fuse = F.interpolate(fuse, size=stage0.size()[2:],mode='bilinear', align_corners=True)
        fuse = F.relu(self.BNDe02(self.ConvDe02(fuse)))
        fuse = self.ConvDe01(fuse)
...
def unit_test():
    net = FEANet(9).cuda(2)
    image = torch.randn(2, 4, 480, 640).cuda(2)
    with torch.no_grad():
